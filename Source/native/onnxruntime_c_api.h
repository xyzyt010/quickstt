...
typedef struct OrtCUDAProviderOptions {
  ...
  OrtCUDAProviderOptions()
      : device_id{}, cudnn_conv_algo_search{OrtCudnnConvAlgoSearchExhaustive},
...
typedef struct OrtROCMProviderOptions {
  ...
  OrtROCMProviderOptions()
      : device_id{}, miopen_conv_exhaustive_search{0}, gpu_mem_limit{SIZE_MAX},
        arena_extend_strategy{}, do_copy_in_default_stream{1},
        has_user_compute_stream{}, user_compute_stream{},
        default_memory_arena_cfg{}, enable_hip_graph{false},
        tunable_op_enable{false}, tunable_op_tuning_enable{false},
        tunable_op_max_tuning_duration_ms{} {}
#endif
...
typedef struct OrtTensorRTProviderOptions {
  ...
  int trt_max_partition_iterations; // maximum iterations for TensorRT parser to
                                    // get capability
...
typedef struct OrtMIGraphXProviderOptions {
  ...
  int migraphx_int8_enable; // MIGraphX INT8 precision. Default 0 = false,
                            // nonzero = true
  int migraphx_use_native_calibration_table; // MIGraphx INT8 cal table. Default
                                             // 0 = false, noznero = true
  const char *migraphx_int8_calibration_table_name; // MIGraphx INT8 calibration
                                                    // table name
...
typedef struct OrtOpenVINOProviderOptions {
  ...
  OrtOpenVINOProviderOptions()
      : device_type{}, enable_npu_fast_compile{}, device_id{}, num_of_threads{},
        cache_dir{}, context{}, enable_opencl_throttling{},
...
struct OrtApi {
   ...
   * api base. The handle to the loaded library is returned in library_handle.
  ...
  /// \name OrtSession
  /// @{
  ORT_CLASS_RELEASE(Session); // Don't call ReleaseSession from Dllmain (because
                              // session owns a thread pool)
  /// @}
  /// \name OrtValue
  /// @{
   ...
   *   "enable_htp_weight_sharing": Enable QNN weight sharing feature while
   * compiling multiple graphs into one QNN context.
   *     - "0": Default. Disabled.
   *     - "1": Enabled.
   ...
   * EP).
   *     - "0": Default. Disabled. QNN EP will handle quantization and
   ...
   * OrtSession is created, then the library handle is released when the
   * provided OrtSessionOptions is released.
   *
   ...
   * could be. it is owned by OrtOptionalTypeInfo instance.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.15.
