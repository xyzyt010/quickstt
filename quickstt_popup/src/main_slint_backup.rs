...
fn commit_text(text: String, output_mode: u8, live_typed_len: usize) {
    ...
    std::thread::spawn(move || {
        if output_mode == 1 {
            typing::copy_only(&trimmed);
        } else if live_typed_len > 0 && trimmed.len() > live_typed_len {
            typing::type_text(&trimmed[live_typed_len..]);
        } else if live_typed_len == 0 {
            typing::type_text(&trimmed);
        }
    });
}
